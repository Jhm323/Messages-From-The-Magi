import{i as e,n as t,o as n,r,s as i,t as a}from"./Footer-Duogp7ad.js";import{o,r as s,s as c,t as l}from"./CardResult-BXQ3wRh0.js";import{n as u,t as d}from"./BirthCardModal-D-5cMoyx.js";import{r as f}from"./SavedReadings-BFfcoB2S.js";import"./SelectionChip-BcFGXq38.js";var p=`modal-compatibility`,m=null;function h(){return`
...
</div>`}function g(){if(!r(p,h()))return;let e=document.getElementById(`${p}-error`);document.getElementById(`${p}-submit`).addEventListener(`click`,()=>{let t=document.getElementById(`${p}-name1`).value.trim()||`Person 1`,n=document.getElementById(`${p}-month1`).value,r=document.getElementById(`${p}-day1`).value,i=document.getElementById(`${p}-name2`).value.trim()||`Person 2`,a=document.getElementById(`${p}-month2`).value,s=document.getElementById(`${p}-day2`).value;if(e.style.display=`none`,!n||!r||!a||!s){e.textContent=`Please select both birthdates to continue.`,e.style.display=`block`;return}let c=o(`${n}/${r}`,`${a}/${s}`);if(!c||!c.card){e.textContent=`Something went wrong. Please check the dates and try again.`,e.style.display=`block`;return}document.getElementById(`${p}-result-container`).innerHTML=l(c.card,{eyebrow:`${t} & ${i}`,subheading:`Your Compatibility Card`,showAffirmation:!0,showAction:!0,showDescription:!0,extra:`<p style="color:var(--color-dawn);font-size:0.8rem;text-align:center;margin-top:0.5rem;">
        Card #${c.card.id} · Numerological value: ${c.reducedValue}
      </p>`}),m={card:c.card,eyebrow:`${t} & ${i}`},_(),document.getElementById(`${p}-step-form`).style.display=`none`,document.getElementById(`${p}-step-result`).style.display=`block`}),document.getElementById(`${p}-again`).addEventListener(`click`,()=>{m=null,document.getElementById(`${p}-step-form`).style.display=`block`,document.getElementById(`${p}-step-result`).style.display=`none`}),document.getElementById(`${p}-save`).addEventListener(`click`,()=>{let e=document.getElementById(`${p}-save-msg`);if(!i()){e.textContent=`Sign in to save your readings.`,e.style.color=`var(--color-gold-muted)`;return}m&&(f({type:`compatibility`,label:`Compatibility Reading`,eyebrow:m.eyebrow,cardId:m.card.id,cardName:m.card.name,cardSuit:m.card.suit,cardSuitSymbol:m.card.suitSymbol}),v())})}function _(){let e=document.getElementById(`${p}-save`),t=document.getElementById(`${p}-save-msg`);e&&(e.textContent=`♦ Save Reading`,e.disabled=!1),t&&(t.textContent=``)}function v(){let e=document.getElementById(`${p}-save`);e&&(e.textContent=`✓ Saved`,e.disabled=!0)}function y(){if(!i())return;let e=n(),t=document.getElementById(`${p}-name1`);if(t&&(t.value=e.name??``),e.birthday){let[,t,n]=e.birthday.split(`-`).map(Number),r=document.getElementById(`${p}-month1`),i=document.getElementById(`${p}-day1`);r&&(r.value=String(t)),i&&(i.value=String(n))}}function b(){g(),e(p),y(),setTimeout(()=>document.getElementById(`${p}-name1`)?.focus(),100)}var x=`modal-geolocation`,S=null;function C(){return`
    ...
    <div id="${x}-step-result" style="display:none;">
      ...
      <div id="${x}-save-msg" style="font-size:0.78rem;text-align:center;color:var(--color-mist);margin-top:0.4rem;min-height:1.2em;"></div>
...
</div>`}function w(){if(!r(x,C()))return;let e=document.getElementById(`${x}-error`);document.getElementById(`${x}-submit`).addEventListener(`click`,()=>{let n=document.getElementById(`${x}-name`).value.trim()||`Seeker`,r=document.getElementById(`${x}-month`).value,i=document.getElementById(`${x}-day`).value,a=document.getElementById(`${x}-location`).value.trim();if(e.style.display=`none`,!r||!i){t(`Please select your birth month and day.`);return}if(!a){t(`Please enter a location name.`);return}let o=c(`${r}/${i}`,a);if(!o||!o.card){t(`Could not calculate the location card. Please check your inputs.`);return}document.getElementById(`${x}-result-container`).innerHTML=l(o.card,{eyebrow:`${n} · ${a}`,subheading:`Your Location Card`,showAffirmation:!0,showAction:!0,showDescription:!0,extra:`
        <div style="margin-top:1rem;padding:0.75rem 1rem;background:rgba(255,255,255,0.03);
          ...
          = Card #${o.reducedValue}
        </div>`}),S={card:o.card,eyebrow:`${n} · ${a}`,location:a},T(),document.getElementById(`${x}-step-form`).style.display=`none`,document.getElementById(`${x}-step-result`).style.display=`block`}),document.getElementById(`${x}-again`).addEventListener(`click`,()=>{S=null,document.getElementById(`${x}-step-form`).style.display=`block`,document.getElementById(`${x}-step-result`).style.display=`none`}),document.getElementById(`${x}-save`).addEventListener(`click`,()=>{let e=document.getElementById(`${x}-save-msg`);if(!i()){e.textContent=`Sign in to save your readings.`,e.style.color=`var(--color-gold-muted)`;return}S&&(f({type:`geolocation`,label:`Location Reading`,eyebrow:S.eyebrow,cardId:S.card.id,cardName:S.card.name,cardSuit:S.card.suit,cardSuitSymbol:S.card.suitSymbol}),E())});function t(t){e.textContent=t,e.style.display=`block`}}function T(){let e=document.getElementById(`${x}-save`),t=document.getElementById(`${x}-save-msg`);e&&(e.textContent=`♦ Save Reading`,e.disabled=!1),t&&(t.textContent=``)}function E(){let e=document.getElementById(`${x}-save`);e&&(e.textContent=`✓ Saved`,e.disabled=!0)}function D(){if(!i())return;let e=n(),t=document.getElementById(`${x}-name`);if(t&&(t.value=e.name??``),e.birthday){let[,t,n]=e.birthday.split(`-`).map(Number),r=document.getElementById(`${x}-month`),i=document.getElementById(`${x}-day`);r&&(r.value=String(t)),i&&(i.value=String(n))}}function O(){w(),e(x),D(),setTimeout(()=>document.getElementById(`${x}-name`)?.focus(),100)}var k=`modal-greeting`,A=null;function j(){return`
...
</div>`}function P(){let e=r(k,j());if(!e)return;let t=document.getElementById(`${k}-error`),n=document.getElementById(`${k}-occasion`),a=e.querySelectorAll(`#${k}-occasion-chips .chip`);a.forEach(e=>{e.addEventListener(`click`,()=>{a.forEach(e=>e.classList.remove(`is-active`)),e.classList.add(`is-active`),n.value=e.dataset.value})}),n.addEventListener(`input`,()=>{a.forEach(e=>e.classList.remove(`is-active`))}),document.getElementById(`${k}-submit`).addEventListener(`click`,()=>{let e=document.getElementById(`${k}-name`).value.trim(),n=document.getElementById(`${k}-month`).value,r=document.getElementById(`${k}-day`).value,i=document.getElementById(`${k}-occasion`).value.trim(),a=document.getElementById(`${k}-message`).value.trim(),c=document.getElementById(`${k}-from`).value.trim();if(t.style.display=`none`,!e){o(`Please enter the recipient's name.`);return}if(!n||!r){o(`Please select the recipient's birth month and day.`);return}let l=s(`${n}/${r}`);if(!l||!l.card){o(`Could not calculate the birth card. Please check the date.`);return}let u=document.getElementById(`${k}-card-output`);u.innerHTML=N({name:e,occasion:i,message:a,from:c,card:l.card}),document.getElementById(`${k}-replay`).addEventListener(`click`,()=>{let e=document.getElementById(`${k}-cover`),t=document.getElementById(`${k}-content`);e.style.animation=`none`,t.style.animation=`none`,t.style.opacity=`0`,e.offsetWidth,e.style.animation=``,t.style.animation=``}),A={card:l.card,eyebrow:e},F(),document.getElementById(`${k}-step-form`).style.display=`none`,document.getElementById(`${k}-step-result`).style.display=`block`}),document.getElementById(`${k}-again`).addEventListener(`click`,()=>{A=null,c()}),document.getElementById(`${k}-save`).addEventListener(`click`,()=>{let e=document.getElementById(`${k}-save-msg`);if(!i()){e.textContent=`Sign in to save your readings.`,e.style.color=`var(--color-gold-muted)`;return}A&&(f({type:`greeting-card`,label:`Greeting Card`,eyebrow:A.eyebrow,cardId:A.card.id,cardName:A.card.name,cardSuit:A.card.suit,cardSuitSymbol:A.card.suitSymbol}),I())}),document.getElementById(`${k}-print`).addEventListener(`click`,()=>{window.print()});function o(e){t.textContent=e,t.style.display=`block`}function c(){document.getElementById(`${k}-step-form`).style.display=`block`,document.getElementById(`${k}-step-result`).style.display=`none`}}function F(){let e=document.getElementById(`${k}-save`),t=document.getElementById(`${k}-save-msg`);e&&(e.textContent=`♦ Save Reading`,e.disabled=!1),t&&(t.textContent=``)}function I(){let e=document.getElementById(`${k}-save`);e&&(e.textContent=`✓ Saved`,e.disabled=!0)}function L(){if(!i())return;let e=n(),t=document.getElementById(`${k}-from`);t&&(t.value=e.name??``)}function R(){P(),e(k),L(),setTimeout(()=>document.getElementById(`${k}-name`)?.focus(),100)}t(`#site-header-mount`,{activePath:`/readings.html`}),a(`#site-footer-mount`),document.body.classList.replace(`js-loading`,`js-ready`);var z={"birth-card":d,compatibility:b,geolocation:O,"greeting-card":R};document.querySelectorAll(`[data-feature]`).forEach(e=>{e.style.cursor=`pointer`,e.addEventListener(`click`,()=>{let t=z[e.dataset.feature];t&&t()})});var B=new URLSearchParams(location.search).get(`open`);B&&z[B]&&z[B]();