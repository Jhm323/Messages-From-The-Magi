import{i as e,o as t,r as n,s as r}from"./Footer-Duogp7ad.js";import{r as i,t as a}from"./CardResult-BXQ3wRh0.js";import{r as o}from"./SavedReadings-BFfcoB2S.js";function s(e,t){return`<div style="display:flex;gap:0.75rem;">
...
</div>`}function d(){if(!n(c,u()))return;let e=document.getElementById(`${c}-error`);document.getElementById(`${c}-submit`).addEventListener(`click`,()=>{let n=document.getElementById(`${c}-name`).value.trim()||`Seeker`,r=document.getElementById(`${c}-month`).value,o=document.getElementById(`${c}-day`).value;if(e.style.display=`none`,!r||!o){t(`Please select your birth month and day.`);return}let s=i(`${r}/${o}`);if(!s||!s.card){t(`Could not calculate your birth card. Please check the date.`);return}document.getElementById(`${c}-result-container`).innerHTML=a(s.card,{eyebrow:n,subheading:`Your Birth Card`,showAffirmation:!0,showAction:!0,showDescription:!0}),l={card:s.card,eyebrow:n},f(),document.getElementById(`${c}-step-form`).style.display=`none`,document.getElementById(`${c}-step-result`).style.display=`block`}),document.getElementById(`${c}-again`).addEventListener(`click`,()=>{l=null,document.getElementById(`${c}-step-form`).style.display=`block`,document.getElementById(`${c}-step-result`).style.display=`none`}),document.getElementById(`${c}-save`).addEventListener(`click`,()=>{let e=document.getElementById(`${c}-save-msg`);if(!r()){e.textContent=`Sign in to save your readings.`,e.style.color=`var(--color-gold-muted)`;return}l&&(o({type:`birth-card`,label:`Birth Card Reading`,eyebrow:l.eyebrow,cardId:l.card.id,cardName:l.card.name,cardSuit:l.card.suit,cardSuitSymbol:l.card.suitSymbol}),p())});function t(t){e.textContent=t,e.style.display=`block`}}function f(){let e=document.getElementById(`${c}-save`),t=document.getElementById(`${c}-save-msg`);e&&(e.textContent=`♦ Save Reading`,e.disabled=!1),t&&(t.textContent=``)}function p(){let e=document.getElementById(`${c}-save`);e&&(e.textContent=`✓ Saved`,e.disabled=!0)}function m(){if(!r())return;let e=t(),n=document.getElementById(`${c}-name`);if(n&&(n.value=e.name??``),e.birthday){let[,t,n]=e.birthday.split(`-`).map(Number),r=document.getElementById(`${c}-month`),i=document.getElementById(`${c}-day`);r&&(r.value=String(t)),i&&(i.value=String(n))}}function h(){d(),e(c),m(),setTimeout(()=>document.getElementById(`${c}-name`)?.focus(),100)}export{s as n,h as t};